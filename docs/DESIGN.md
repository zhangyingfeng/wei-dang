...
- **正文抓取的瞬时失败**：10 个失败里有 3 篇文章报的是通用的"未能在文章页面中找到正文"错误，但同一个 URL 几分钟后手动重新抓取完全正常——说明是瞬时网络/CDN 抖动，不是页面结构问题。之前 `fetchBody` 只请求一次，一次失败就整项标记为失败。已经参照图片下载已有的重试逻辑，给 `fetchBody` 加了同样的重试（3 次、指数退避），但没有重试 `SessionExpiredError`（那是确定性状态，重试没有意义）。
...
**后来换成了用户自己找的设计**（2026-09-18）：文件夹图形 + "微档"两字，同样是微信绿。给出了两版"去掉文字、保留文件夹意象"的简化替代方案（`已删除，仅存在过这次会话里`）供参考，但用户明确选择保留完整的"微档"文字版。处理过程：

- 源图（用户提供，`~/Downloads/微档.png`，1254×1254，纯白背景无透明通道）先用"从四角种子点泛色填充"的方式把白色背景抠成透明——不是全局删除所有白色像素，这样能保留图标内部本来就是白色的文件夹图形，不会把它一起挖空。
- 抠出来的图形本身不是完全正方形（1196×1212），也没有严格贴合 Apple 官方图标模板的边距/圆角比例——按知档 `apply-apple-icon-spec.py` 同样的思路重新处理：缩放到 824×824，重新画一个精确 185.4px 半径的圆角蒙版（不信任源图自己的圆角，用代码重新算），居中贴到 1024×1024 画布上，四周留出精确的 100px 边距。
- **已知取舍**：32px（菜单栏/Finder 小图标常见尺寸）下"微档"两个字基本无法辨认，只能看出是个绿色文件夹形状——这一点在选择前已经跟用户说明过，用户认为文件夹意象本身足够辨识，接受这个权衡。和知档的极简"纯色块+单字"视觉语言也不再统一，是两个项目图标风格分道扬镳的一个有意选择。

**修了一个自己引入的圆角边缘发黑的 bug**（2026-09-18，用户目测发现）：上面"重新画精确圆角蒙版"这一步是问题根源。Pillow 11.3.0 的 `Image.resize()` 在用 `LANCZOS`/`BILINEAR` 这类插值滤镜时，会自动按 alpha 预乘再还原（这是好事，本来是为了避免非预乘 alpha 缩放常见的白边/黑边问题）——但这意味着源图里"完全透明"的像素，缩放后 RGB 会被还原成 `(0,0,0)`（因为透明像素本来就不携带有意义的颜色信息，正常情况下无所谓）。问题出在下一步：又重新画了一个跟源图实际内容形状不完全对齐的、独立计算出来的圆角蒙版，把这个新蒙版的 alpha 强行套在缩放后的 RGB 上——在两个形状对不齐的地方，蒙版说"这里该显示"，但对应位置的 RGB 其实是刚才被清零的 `(0,0,0)`，于是那圈错位的地方就变成了半透明的黑色，圆角边缘看起来像是蒙了一层脏东西。

修法很直接：不再自己重新计算圆角蒙版，直接信任源图自己缩放后的 alpha 通道（本来就是干净的、和内容对齐的）。裁剪 → 缩放到 824×824 → 直接贴到 1024×1024 画布，中间不再插入"重新算圆角"这一步。缩放后单独验证了 824×824 图里所有半透明（0＜alpha＜255）像素的 RGB 值，确认都是合理的、和图标本身配色一致的过渡色，不再有黑色像素残留。
...
8. ~~发布 0.1.0。~~ 已完成（2026-09-18）：打了 `v0.1.0` 标签，`gh release create` 建了 GitHub Release，附上了签名+公证过的 `.dmg`。发布当时仓库还是 private，Release 只对受邀协作者可见；仓库本身在 2026-09-18 晚些时候改成了 public（见下一条），现在任何人都能看到仓库、下载这个 Release。踩了一个小坑：`gh release create`/`gh release upload` 会把文件名里的中文字符完全丢掉（不是编码错误，是直接消失），最后按知档的既有习惯把 `.dmg` 文件名改成纯 ASCII（`wei-dang_0.1.0_aarch64.dmg`）才正常。
9. ~~仓库改成 public。~~ 已完成（2026-09-18）：`gh repo edit --visibility public --accept-visibility-change-consequences`。改之前确认过仓库只有自己一个协作者（受邀协作者可见 = 只有自己能看到），也确认过 git 历史里没有真实凭据或其它敏感内容残留（见"清理测试文件里的真实 token"那次提交）。

## 再往后

- 补上验证码/风控页面的识别和清晰的错误提示——目前几次真实端到端运行都没有触发过风控，`parsePublishPage` 对非 `ret:0` 响应的分类还是猜测，没有真实样本可以核对。
- 要不要整理一份类似知档 `docs/RELEASE_CHECKLIST.md` 的正式发布清单，留给下一次发版本时决定。
